...
export default {
	name: "OperationsAssetEquipmentInventory",
	meta: {
		sort: 2,
		title: "盘点",
		hideMenu: true,
	...
	methods: {
			...
			return (
				<div class="card-item plan-item">
					...
					<div class="card-footer">
						...
						{item.inventoryResult == 3 ? (
							<span type="text" onClick={() => this.Input(item)}>
								录入
							</span>
						) : (
							""
		...
		showDetail(id) {
			this.$router.push({
				path: "/operations/asset/equipment/inventory/detail",
				query: { inventoryId: id },
			})
		},
		showTable() {
			this.$router.push({
				path: "/operations/asset/equipment/inventory/allTable",
			})
		},
	},
}
