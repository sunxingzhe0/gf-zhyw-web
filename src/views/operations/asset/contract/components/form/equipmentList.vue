...
export default {
	...
	mixins: [
		TableMixin([
			{
				fetchListFunction: asset.contract.getequipmentList,
			},
		]),
	],
	data() {
	...
	methods: {
		search() {
			this.$_fetchTableData(asset.contract.getequipmentList)
		},
		async setLians() {
			let ids = []
			...
			await asset.contract.saveEquipment({ equipments: ids.join(",") })
		...
		async removeLians() {
			...
			await asset.contract.removeEquipment({ equipments: ids.join(",") })
			this.$message.success("操作成功")
			this.search()
			this.$refs.table2.$_fetchTableData(asset.contract.getequipmentList)
		},
	},
}
</script>
