...
export default {
	...
	mixins: [
		TableMixin([
			{
				fetchListFunction: asset.supplier.getcontractList,
			},
		]),
	],
	data() {
	...
	methods: {
		...
		async setLians() {
			...
			await asset.supplier.saveContracts({ contracts: ids.join(",") })
			this.$message.success("操作成功")
			this.search()
			this.$refs.table2.$_fetchTableData(asset.supplier.getcontractList)
		},
		async removeLians() {
			let ids = []
			...
			await asset.supplier.removeContracts({ contracts: ids.join(",") })
			this.$message.success("操作成功")
			this.search()
			this.$refs.table2.$_fetchTableData(asset.supplier.getcontractList)
		},
