...
<template>
  <div>
    ...
    <Lists>
      <div class="currency-card-item" v-for="(item, index) in lists" :key="index" @click="openDetail(item)">
...
export default {
  ...
  methods: {
    ...
    async getList() {
      this.loading = true;
      let res = await stock.goodsOut.goodsOutList({});
      this.lists = res.list;
