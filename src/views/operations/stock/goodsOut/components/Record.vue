  ...
  <div class="inDialog">
    <el-row class="content">
      <el-col :span="8" class="item"
        ...
        >{{ detail.outType == 1 ? "领用" : "报损" }}</el-col
      >
      <el-col :span="8" class="item"
        ><span class="title">出货仓库：</span>{{ detail.outStore }}</el-col
      >
      ...
      >
      <el-col :span="8" class="item" v-if="!isReview"
        ><span class="title">出库数量：</span>{{ detail.number }}</el-col
      >
      <el-col :span="8" class="item" v-if="!isReview"
        ><span class="title">总价：</span>￥{{ detail.allPrice }}</el-col
      >
      <el-col :span="8" class="item"
        ><span class="title">出库人：</span>{{ detail.outUser }}</el-col
      >
      <el-col :span="8" class="item"
        ><span class="title">出库时间：</span>{{ detail.outTime }}</el-col
      >
      <el-col :span="8" class="item"
      ...
      >
      <el-col :span="8" class="item" v-if="detail.outType == 2"
        ><span class="title">报损理由：</span>{{ detail.reason }}</el-col
      >
    </el-row>
...
export default {
  ...
  methods: {
    ...
    async goodsOutRecord() {
      let res = await stock.goodsOut.goodsOutRecord({ id: this.id });
      console.log(res);
      this.detail = res;
