...
<template>
  <div>
    ...
    <DarkDialog v-model="moreDialog">
      <MoreReview :propsCurrent="'3'"></MoreReview>
    </DarkDialog>
  </div>
</template>
<script>
...
import Record from "../../goodsOut/components/Record";
...
export default {
  name: "OperationsStockgoodsReviewOutStore",
  meta: {
    title: "出库",
    hideMenu: true,
  },
  ...
  methods: {
    ...
    async getList() {
      this.loading = true;
      let res = await stock.goodsReview.outStoreReviewList({});
      this.lists = res.list;
